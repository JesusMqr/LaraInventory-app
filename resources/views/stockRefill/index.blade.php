...
@section('content')
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    @foreach ($refillRequest as $request )
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="p-4">
                                <img src="{{ $request->product->image_url}}" class="w-16 md:w-32 max-w-full max-h-full" alt="{{ $request->product->name }}">
                            </td>
                            ...
                            <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                                @switch($request->status)
                                    @case('pendiente')
                                        <span class="bg-yellow-100 truncate text-yellow-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-yellow-900 dark:text-yellow-300">{{ $request->status }}</span>
                                    @break
                                    @case('en proceso')
                                        <span class="bg-blue-100 truncate text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{$request->status}}</span>

                                    @break
                                    @case("completo")
                                        <span class="bg-green-100 truncate text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">{{ $request->status }}</span>
                                        @break
                                    @case("rechazado")
                                        <span class="bg-red-100 truncate text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">{{ $request->status }}</span>
                                        @break
                                    @default
        ...
        @else
        <div>
            <h2 class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
